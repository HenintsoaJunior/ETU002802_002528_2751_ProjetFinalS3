    ...
    function traiterReponse(response, erreur) {
        if (erreur) {
            ...
            if (response.success) {
                ...
                if (pagetype === 'update') {
                    window.location.href = 'prixVente.html';
                }
...
function chargerListParcelleVarieteAff() {
    ...
    createXHR('GET', '../../model/backoffice/TraitementListePrixVente.php', null, traiterReponse);
}


function createParcelleVarieteTable(response) {
    const divListeVariete = document.getElementById('listePrixVente');
    ...
    if (Array.isArray(response)) {
        ...
        response.forEach(item => {
            ...
            tableHTML += `<tr><td>${item.idVarieteThe}</td><td>${item.prix}</td><td><button class="buttonUpdate">Update</button></td><td><button class="buttonDelete">Delete</button></td></tr>`;
        });
        ...
        updateButtons.forEach(button => {
            button.addEventListener('click', function() {
                ...
                showPreview(response[selectedIndex].idVarieteThe, response[selectedIndex].prix,response[selectedIndex].id_prixVenteThe);
            });
        });
        ...
        deleteButtons.forEach(button => {
            ...
            const id_prixVenteThes = response[selectedIndex].id_prixVenteThe;
            preparerDonneesDeletePrixVente(id_prixVenteThes);
...
function showPreview(idVarieteThe,prix, id) {
    ...
    const optionsParcelle = response.map(item => `<option value="${item.id_parcelleThe}" ${listeParcelleVariete.idParcelleThe === item.id_parcelleThe ? 'selected' : ''}>${item.id_parcelleThe}</option>`).join('');
...
function preparerDonneesListVarieteAff(idParcelleThe,idVarieteThe,id) {
    ...
    envoyerFormulaireAuServeur(data, '../../model/backoffice/TraitemantUpdatePrixVente.php', 'update');
}
...
function chargerListParcelleVariete() {
    ...
    createXHR('GET', '../../model/backoffice/TraitementListeV.php', null, traiterReponse);
}

function createParcelleSelect(response) {
    const divListeParcelle = document.getElementById('InsertPrixVente');

    if (response && response.listeVariete) {
        // Effacer le contenu précédent de divListeParcelle
        divListeParcelle.innerHTML = '';

        const selectHTMLVariete = '<label for="selectVariete">Variété : </label>' +
            '<select id="selectVariete">' +
            response['listeVariete'].map(item => `<option value="${item.id_varieteThe}">${item.nom}</option>`).join('') +
            '</select>';

            const prix = '<label for="prix">prix :</label>' +
        ...
        divListeParcelle.innerHTML = selectHTMLVariete + '<br>' + prix + '<br>';
...
function preparerDonneesPrixVente(idVarieteThe, prix) {
    const formData = {
        idVarieteThe: idVarieteThe,
        prix: prix
    };
    ...
    envoyerFormulaireAuServeur(data, '../../model/backoffice/TraitementinsertPrixVente.php', 'insertPrixVente');
}
...
function preparerDonneesDeletePartelleVariete(id) {
    ...
    envoyerFormulaireAuServeur(data, '../../model/backoffice/TraitementDeletePrixVente.php', 'delete');
}
